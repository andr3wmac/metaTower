...
class SearchEngine():
    ...
    def query(self, content):
        if ( self.query_fn ): return self.query_fn(content)
        return []
...
def query(httpOut, content, engineIndex = -1):
    global engines
    results = {}
    ...
    for engine in engines:
        if ( engineIndex > -1 ):
            if ( engineIndex == engineNum ): results[engineNum] = engine.query(content)
            else: results[engineNum] = []
        else:
            results[engineNum] = engine.query(content)            
        engineNum += 1
